  ...
  notFoundComponent: () => (
    <div className="min-h-screen flex items-center justify-center font-mono text-bone/60">
      <div className="text-center">
        ...
        <Link to="/" className="border border-acid text-acid px-4 py-2 hover:bg-acid hover:text-ink transition">return ←</Link>
      </div>
    </div>
  ),
...
const MODULES: Record<string, ModuleDef> = {
  "surface-scanner": {
    ...
    capabilities: [
      { title: "OWASP API Top 10", desc: "BOLA, broken auth, excessive data, mass assignment, SSRF — all mapped to endpoint contracts." },
      { title: "CVE Cross-Match", desc: "Continuously diffs your runtime dependencies against NVD + GitHub advisories." },
      { title: "Auth Bypass Probes", desc: "Token replay, scope confusion, IDOR walks, JWT alg-none, expired-token acceptance." },
      { title: "Live Severity Score", desc: "0–100 per endpoint with decay curve. Score drops the moment a regression lands." },
    ],
    feed: [
  ...
  "openapi-inhaler": {
    ...
    capabilities: [
      { title: "Universal Importer", desc: "Drag, drop, paste, or webhook. Inhales OpenAPI 2/3/3.1, Postman 2.1, HAR, curl logs." },
      { title: "Canonical Normalizer", desc: "Collapses synonyms, infers missing schemas, resolves $refs, scrubs PII." },
      { title: "Spec Diff Engine", desc: "Structural + semantic diff. Knows the difference between rename and remove." },
      { title: "Auto-Reconciliation", desc: "When two specs disagree, picks the one your runtime actually serves." },
    ],
    feed: [
  ...
  "repo-bridge": {
    ...
    metrics: [
      ...
      { label: "ci providers", value: "6", sub: "actions · jenkins · gitlab · circle · buildkite · drone" },
    ],
    ops: [
    ...
    capabilities: [
      { title: "GitHub Native App", desc: "Installed once per org. PR checks, inline annotations, suggested patches." },
      { title: "Jenkins Step", desc: "Drop-in pipeline step: apiguardScan(profile: 'strict'). Junit + sarif output." },
      { title: "Branch Protection", desc: "Block merges below your security score floor. No exceptions, no DMs." },
      { title: "Spec Drift Bot", desc: "Opens a PR when shipped runtime diverges from the committed spec." },
    ],
    feed: [
  ...
  "forensic-pdf": {
    ...
    capabilities: [
      { title: "Evidence Bundling", desc: "Each finding ships with the exact request, response, headers, and replay curl." },
      { title: "CVSS + OWASP Mapping", desc: "Cross-referenced to OWASP API Top 10, CWE, MITRE ATT&CK technique IDs." },
      { title: "Compliance Templates", desc: "SOC2, ISO 27001 Annex A, PCI-DSS, HIPAA Security Rule — sectioned and indexed." },
      { title: "Cryptographic Signing", desc: "PAdES-LTA with RFC 3161 timestamp. Auditors verify offline." },
    ],
    feed: [
  ...
  "change-radar": {
    ...
    capabilities: [
      { title: "Runtime Taps", desc: "Sidecar or eBPF probe captures shipped behavior, not just declared spec." },
      { title: "Semantic Diff", desc: "Knows that renaming `user_id` → `userId` is one change, not two." },
      { title: "Blast Radius", desc: "Walks the consumer graph and tells you exactly which clients will break." },
      { title: "Triage Channels", desc: "Routes by service owner — Slack thread, PagerDuty incident, GitHub issue." },
    ],
    feed: [
  ...
  "health-pulse": {
    ...
    capabilities: [
      { title: "Multi-Region Probes", desc: "14 PoPs. Real TCP, real TLS, real DNS — not just ICMP. Anycast-aware." },
      { title: "Latency Histograms", desc: "Full distribution. p50, p95, p99, p99.9. Tail latency, not just averages." },
      { title: "Error Budget Burn", desc: "Multi-window, multi-burn-rate alerts. Pages on slope, not just symptom." },
      { title: "Synthetic Journeys", desc: "Scripted user flows — login → search → checkout — run every 60s." },
    ],
    feed: [
  ...
  return (
    <div className="relative bg-transparent text-foreground min-h-screen">
      ...
      <header className="fixed top-0 inset-x-0 z-50 border-b border-border/60 bg-background/70 backdrop-blur-xl">
        <div className="flex items-center justify-between px-6 py-3 font-mono text-xs tracking-widest uppercase">
          <div className="flex items-center gap-3">
            ...
            <Link to="/" className="text-bone hover:text-acid">APIGUARD/<span style={{ color: m.accent }}>{m.tag}</span></Link>
            <span className="text-bone/40 hidden md:inline">// mod.{m.code} · {m.title.toLowerCase()}</span>
          </div>
          <Link to="/" className="border border-bone/40 text-bone px-3 py-1 hover:border-acid hover:text-acid transition">
            ← all.modules
          </Link>
        </div>
      </header>
      ...
      <section className="relative grid-bg pt-32 pb-20 overflow-hidden">
        ...
        <div className="absolute top-0 left-0 right-0 h-px" style={{ background: `linear-gradient(to right, transparent, ${m.accent}, transparent)` }} />
        ...
        <div className="relative z-10 max-w-[1400px] mx-auto px-6">
          ...
          >
            <Link to="/" className="hover:text-acid">~/modules</Link>
          ...
          <div className="grid md:grid-cols-[1.6fr_1fr] gap-12 items-end">
            <div>
              ...
              >
                <span className="font-display text-7xl leading-none" style={{ color: m.accent }}>{m.glyph}</span>
            ...
            >
              ...
              {m.cli.map((line, i) => (
                <motion.div
                  ...
                  className={line.startsWith("✓") ? "" : line.startsWith("▸") ? "text-bone" : "text-bone/50"}
      ...
      <section className="border-y border-border bg-background/40 backdrop-blur-xl">
        <div className="max-w-[1400px] mx-auto grid grid-cols-2 md:grid-cols-4 divide-x divide-border">
          {m.metrics.map((x, i) => (
            ...
            >
              <div className="font-mono text-[10px] text-bone/40 uppercase tracking-widest mb-3">{x.label}</div>
              <div className="font-display text-4xl md:text-5xl tracking-tight" style={{ color: m.accent }}>{x.value}</div>
      ...
      <section className="max-w-[1400px] mx-auto px-6 py-24 grid md:grid-cols-[1.5fr_1fr] gap-12">
        <div>
          <div className="font-mono text-xs uppercase tracking-widest mb-4" style={{ color: m.accent }}>[ 01 ] dossier</div>
        ...
        <div className="border border-border bg-card/40 backdrop-blur-xl p-6">
          ...
          <div className="space-y-3 font-mono text-xs">
            {m.ops.map((o) => (
              <div key={o.k} className="grid grid-cols-[1fr_1.5fr] gap-3 border-b border-border/50 pb-2">
      ...
      <section className="max-w-[1400px] mx-auto px-6 py-16">
        <div className="border-b border-border pb-6 mb-10 flex items-end justify-between flex-wrap gap-4">
          <div>
            <div className="font-mono text-xs uppercase tracking-widest mb-3" style={{ color: m.accent }}>[ 02 ] capabilities</div>
            <h2 className="font-display text-4xl md:text-5xl uppercase tracking-tight">What it actually does.</h2>
          </div>
          ...
          {m.capabilities.map((c, i) => (
            ...
            >
              <div className="flex items-start justify-between mb-4">
                ...
                <span className="text-2xl" style={{ color: m.accent }}>{["◇", "◈", "◆", "▣"][i]}</span>
              </div>
              <h3 className="font-display text-2xl uppercase tracking-tight mb-3 group-hover:text-acid transition-colors">{c.title}</h3>
      ...
      <section className="max-w-[1400px] mx-auto px-6 py-24">
        <div className="border-b border-border pb-6 mb-10 flex items-end justify-between flex-wrap gap-4">
          <div>
            <div className="font-mono text-xs uppercase tracking-widest mb-3" style={{ color: m.accent }}>[ 03 ] live.stream</div>
            <h2 className="font-display text-4xl md:text-5xl uppercase tracking-tight">As it happens.</h2>
          </div>
          <div className="font-mono text-[10px] text-bone/40 flex items-center gap-2">
            <span className="size-1.5 rounded-full animate-pulse" style={{ background: m.accent }} />
            tailing · 5m window
          </div>
        </div>
      ...
      <section className="max-w-[1400px] mx-auto px-6 py-24">
        <div className="border border-border bg-card p-10 md:p-16 text-center relative overflow-hidden">
          ...
          <div className="relative">
            <div className="font-mono text-xs uppercase tracking-widest mb-4" style={{ color: m.accent }}>[ engage ]</div>
            <h2 className="font-display text-4xl md:text-6xl uppercase tracking-tight leading-[0.9] mb-8">
              Wire <span className="italic font-serif lowercase" style={{ color: m.accent }}>{m.title.toLowerCase()}</span><br />
