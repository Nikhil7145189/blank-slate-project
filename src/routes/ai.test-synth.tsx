...
export const Route = createFileRoute("/ai/test-synth")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Generate negative, fuzz, boundary and auth-bypass test cases from any OpenAPI spec. Export to Jest, Pytest, k6." },
    ],
  }),
...
const CASE_CLASSES = [
  { code: "NEG", title: "Negative", desc: "Reject malformed payloads, wrong types, missing required fields.", count: 412, tone: "var(--acid)" },
  { code: "FZZ", title: "Fuzz", desc: "Mutational + grammar-based payloads against every field.", count: 9_318, tone: "var(--signal)" },
  { code: "BND", title: "Boundary", desc: "Off-by-one, integer overflow, string length walls, unicode edges.", count: 1_204, tone: "var(--warn)" },
  { code: "AUT", title: "Auth-Bypass", desc: "BOLA, BFLA, IDOR, JWT-none, scope confusion, role escalation.", count: 287, tone: "var(--signal)" },
  { code: "RCE", title: "Injection", desc: "SQLi, NoSQLi, SSRF, template, command, header smuggling.", count: 642, tone: "var(--warn)" },
  { code: "RAT", title: "Rate / Quota", desc: "Burst, drip, distributed, refill timing, quota leak.", count: 96, tone: "var(--acid)" },
];
...
const STREAM = [
  ...
  { t: "00:00.41", line: "▸ inferring auth model · oauth2 + bearer · 4 scopes", tone: "var(--bone)" },
  { t: "00:01.07", line: "✦ synthesizing NEG class · 412 cases", tone: "var(--acid)" },
  { t: "00:02.55", line: "✦ synthesizing FZZ class · 9,318 cases (mutational)", tone: "var(--signal)" },
  { t: "00:04.21", line: "⚠ POST /v3/checkout · missing rate-limit · auth-bypass risk", tone: "var(--warn)" },
  { t: "00:05.93", line: "✦ synthesizing AUT class · BOLA on GET /users/{id}/orders", tone: "var(--signal)" },
  ...
  return (
    <div className="relative bg-background text-foreground">
      ...
      <header className="fixed top-0 inset-x-0 z-50 border-b border-border/60 bg-background/70 backdrop-blur-xl">
        <div className="flex items-center justify-between px-6 py-3 font-mono text-xs tracking-widest uppercase">
          <div className="flex items-center gap-3">
            ...
            <Link to="/" className="text-bone hover:text-acid">APIGUARD/<span className="text-signal">ai.tests</span></Link>
            <span className="text-bone/40 hidden md:inline">// synthesizer v2.7 · gpt-augmented · 47ms</span>
          </div>
      ...
      <section ref={heroRef} className="relative min-h-[110vh] grid-bg overflow-hidden pt-28">
        ...
        <motion.div style={{ rotate: rotGlyph }} className="absolute right-[-180px] top-[20%] size-[520px] border border-signal/30 rounded-full pointer-events-none" />
        <motion.div style={{ rotate: useTransform(scrollYProgress, [0, 1], [360, 0]) }} className="absolute right-[-80px] top-[28%] size-[360px] border border-acid/20 rounded-full pointer-events-none" />

        <motion.div style={{ y: yTitle, opacity: opTitle }} className="relative px-6 max-w-[1400px] mx-auto pt-12">
          <div className="font-mono text-xs text-signal uppercase tracking-[0.3em] mb-6">[ ai.module · 02/03 · test case synthesizer ]</div>
          ...
          <p className="mt-10 max-w-2xl text-bone/60 text-lg leading-relaxed">
            ...
            forgot to imagine — boundary edges, malformed unicode, scope-confused
            JWTs, BOLA chains, refill-timing races — and ships them as runnable
            Jest, Pytest or k6 files.
          </p>
          ...
          <div className="mt-12 flex flex-wrap items-center gap-4">
            ...
            <span className="font-mono text-xs text-bone/40">expected · ~7s · ~11,959 cases · 2.1 MB</span>
          </div>
        </motion.div>
            ...
            <div className="grid md:grid-cols-[1fr_auto_1fr] gap-0">
              <div className="p-6 font-mono text-xs space-y-3">
                ...
                <div className="flex flex-wrap gap-2">
                  {EXPORTS.map((x, i) => (
                    <span key={x} className={`border px-2 py-1 cursor-pointer transition ${i < 3 ? "border-signal text-signal" : "border-bone/20 text-bone/40 hover:border-bone/60 hover:text-bone/70"}`}>
                      {x}
                    </span>
                  ))}
                </div>
              ...
              <div className="p-6 font-mono text-xs space-y-1 bg-background/40 max-h-[280px] overflow-hidden">
                ...
                <div className="text-acid">▸ ready<span className="cursor-blink">_</span></div>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="relative px-6 max-w-[1400px] mx-auto py-32">
        <div className="flex items-end justify-between mb-12 flex-wrap gap-4">
          <div>
            <div className="font-mono text-xs text-signal uppercase tracking-[0.3em] mb-3">[ 02 · case.classes ]</div>
            <h2 className="font-display text-5xl md:text-7xl uppercase tracking-tighter leading-[0.9]">six classes.<br/><span className="text-stroke">one synth pass.</span></h2>
          </div>
            ...
            <TiltCard key={c.code} className="group">
              ...
              >
                ...
                <div className="relative flex items-start justify-between mb-8">
                  <span className="font-mono text-xs uppercase tracking-widest" style={{ color: c.tone }}>[ {c.code} ]</span>
                  <span className="size-2 rounded-full animate-pulse" style={{ background: c.tone }} />
                </div>
                <h3 className="relative font-display text-3xl uppercase tracking-tight mb-3">{c.title}</h3>
                ...
                <div className="relative flex items-baseline justify-between border-t border-border pt-4 font-mono text-xs">
                  ...
                  <span style={{ color: c.tone }} className="text-2xl font-display">{c.count.toLocaleString()}</span>
                </div>
              </motion.div>
            </TiltCard>
          ))}
          ...
          <div className="bg-background/60 font-mono text-xs overflow-hidden">
            <div className="flex border-b border-border">
              {["auth_bypass.test.ts", "fuzz_checkout.py", "load_burst.k6.js"].map((f, i) => (
                <div key={f} className={`px-4 py-3 border-r border-border ${i === 0 ? "bg-card text-acid" : "text-bone/40"}`}>{f}</div>
              ))}
            </div>
...
// `}<span className="text-signal">// synthesized · risk 0.91 · novelty 0.77</span>
      ...
      <section className="relative px-6 max-w-[1400px] mx-auto pb-32">
        ...
        <div className="border border-border bg-card/50">
          ...
          {[
            { c: "AUT", e: "GET /v3/orders/{id}", r: 0.91, n: 0.77, s: "FAIL", tone: "var(--signal)" },
            { c: "FZZ", e: "POST /v3/checkout/express", r: 0.84, n: 0.92, s: "FAIL", tone: "var(--signal)" },
            { c: "BND", e: "PATCH /users/{id}/quota", r: 0.71, n: 0.34, s: "PASS", tone: "var(--acid)" },
            { c: "RCE", e: "POST /admin/sql/export", r: 0.96, n: 0.41, s: "FAIL", tone: "var(--signal)" },
            { c: "NEG", e: "PUT /catalog/{sku}", r: 0.52, n: 0.18, s: "PASS", tone: "var(--acid)" },
            { c: "RAT", e: "POST /auth/refresh", r: 0.68, n: 0.81, s: "FLAKY", tone: "var(--warn)" },
          ].map((row, i) => (
            <motion.div
              key={i}
            ...
            >
              ...
              <div className="p-3">
                <div className="h-1 bg-bone/10 relative">
                  <div className="absolute inset-y-0 left-0 bg-signal" style={{ width: `${row.r * 100}%` }} />
                </div>
                <div className="text-bone/40 mt-1">{row.r}</div>
              </div>
              <div className="p-3">
                <div className="h-1 bg-bone/10 relative">
                  <div className="absolute inset-y-0 left-0 bg-acid" style={{ width: `${row.n * 100}%` }} />
                </div>
                <div className="text-bone/40 mt-1">{row.n}</div>
              </div>
              <div className="p-3 text-right" style={{ color: row.tone }}>{row.s}</div>
            </motion.div>
          ))}
        </div>
      </section>
      ...
      <section className="border-t border-border bg-card/30">
        <div className="max-w-[1400px] mx-auto px-6 py-16 grid md:grid-cols-3 gap-8">
          <div>
            ...
            <Link to="/" className="font-display text-2xl uppercase hover:text-acid transition">← AI.Docs · generator</Link>
          </div>
          <div className="text-center">
            <div className="font-mono text-xs text-bone/40 uppercase tracking-widest mb-2">return</div>
            <Link to="/" className="font-display text-2xl uppercase hover:text-signal transition">// apiguard.os</Link>
          </div>
          <div className="md:text-right">
            <div className="font-mono text-xs text-bone/40 uppercase tracking-widest mb-2">next module</div>
            <Link to="/" className="font-display text-2xl uppercase hover:text-acid transition">AI.Mock · server →</Link>
          </div>
        </div>
