...
export const Route = createFileRoute("/ai/docs-gen")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Point at a base URL. Get a fully-narrated, sectioned docs site in 90 seconds — endpoints, examples, edge cases." },
    ],
  }),
...
const SECTIONS = [
  { code: "OVR", title: "Overview", desc: "Auto-written prose: what the API does, who it's for, auth model, conventions.", lines: 184, tone: "var(--acid)" },
  { code: "REF", title: "Endpoint Reference", desc: "Every route, method, param, header, body, status — fully cross-linked.", lines: 9_412, tone: "var(--signal)" },
  { code: "EXM", title: "Examples", desc: "curl, fetch, axios, requests, httpx, Go, Rust — all generated, all runnable.", lines: 2_087, tone: "var(--acid)" },
  { code: "EDG", title: "Edge Cases", desc: "Null bodies, 429 storms, partial writes, idempotency keys, retries.", lines: 612, tone: "var(--warn)" },
  { code: "AUT", title: "Auth Flows", desc: "OAuth2, PKCE, mTLS, API keys — diagrammed and narrated end-to-end.", lines: 318, tone: "var(--signal)" },
  { code: "CHG", title: "Changelog", desc: "Versioned diff prose. \"What changed and why you care.\" Auto-pinned.", lines: 144, tone: "var(--acid)" },
];

const FORMATS = ["html", "markdown", "mdx", "docusaurus", "mintlify", "pdf", "openapi", "redoc"];

const STREAM = [
  { t: "00:00.08", line: "▸ probing https://api.acme.io · 247 endpoints found", tone: "var(--acid)" },
  { t: "00:00.31", line: "▸ inferring auth · oauth2 + bearer · 4 scopes detected", tone: "var(--bone)" },
  ...
  { t: "00:07.55", line: "✦ generating examples · 7 languages · 2,087 snippets", tone: "var(--signal)" },
  { t: "00:11.83", line: "⚠ POST /v3/checkout · ambiguous error envelope · documented both", tone: "var(--warn)" },
  { t: "00:14.27", line: "✓ docs site built · 12,757 lines · 4.8 MB · ready to publish", tone: "var(--acid)" },
];
  ...
  return (
    <div className="relative bg-background text-foreground">
      ...
      <header className="fixed top-0 inset-x-0 z-50 border-b border-border/60 bg-background/70 backdrop-blur-xl">
        <div className="flex items-center justify-between px-6 py-3 font-mono text-xs tracking-widest uppercase">
          <div className="flex items-center gap-3">
            ...
            <Link to="/" className="text-bone hover:text-acid">APIGUARD/<span className="text-acid">ai.docs</span></Link>
            <span className="text-bone/40 hidden md:inline">// generator v3.1 · llm-augmented · 47ms</span>
          </div>
      ...
      <section ref={heroRef} className="relative min-h-[110vh] grid-bg overflow-hidden pt-28">
        ...
        <motion.div style={{ rotate: rotGlyph }} className="absolute right-[-180px] top-[20%] size-[520px] border border-acid/30 rounded-full pointer-events-none" />
        <motion.div style={{ rotate: rotGlyph2 }} className="absolute right-[-80px] top-[28%] size-[360px] border border-signal/20 rounded-full pointer-events-none" />

        <motion.div style={{ y: yTitle, opacity: opTitle }} className="relative px-6 max-w-[1400px] mx-auto pt-12">
          <div className="font-mono text-xs text-acid uppercase tracking-[0.3em] mb-6">[ ai.module · 01/03 · documentation generator ]</div>
          ...
          <div className="mt-12 flex flex-wrap items-center gap-4">
            ...
            <span className="font-mono text-xs text-bone/40">expected · ~14s · ~12,757 lines · 4.8 MB</span>
          </div>
        </motion.div>
            ...
            <div className="grid md:grid-cols-[1fr_auto_1fr] gap-0">
              <div className="p-6 font-mono text-xs space-y-3">
                ...
                <div className="flex flex-wrap gap-2">
                  {FORMATS.map((x, i) => (
                    <span key={x} className={`border px-2 py-1 cursor-pointer transition ${i < 4 ? "border-acid text-acid" : "border-bone/20 text-bone/40 hover:border-bone/60 hover:text-bone/70"}`}>
                      {x}
                    </span>
                  ))}
                </div>
                ...
                <div className="flex flex-wrap gap-2">
                  {["technical", "friendly", "terse", "verbose"].map((t, i) => (
                    <span key={t} className={`border px-2 py-1 cursor-pointer transition ${i === 0 ? "border-signal text-signal" : "border-bone/20 text-bone/40 hover:border-bone/60 hover:text-bone/70"}`}>
                      {t}
                    </span>
                  ))}
                </div>
      ...
      <section className="relative px-6 max-w-[1400px] mx-auto py-32">
        <div className="flex items-end justify-between mb-12 flex-wrap gap-4">
          <div>
            <div className="font-mono text-xs text-acid uppercase tracking-[0.3em] mb-3">[ 02 · sections ]</div>
            <h2 className="font-display text-5xl md:text-7xl uppercase tracking-tighter leading-[0.9]">six sections.<br/><span className="text-stroke">one narration pass.</span></h2>
          </div>
            ...
            <TiltCard key={s.code} className="group">
              ...
              >
                ...
                <div className="relative flex items-start justify-between mb-8">
                  <span className="font-mono text-xs uppercase tracking-widest" style={{ color: s.tone }}>[ {s.code} ]</span>
                  <span className="size-2 rounded-full animate-pulse" style={{ background: s.tone }} />
                </div>
                <h3 className="relative font-display text-3xl uppercase tracking-tight mb-3">{s.title}</h3>
                ...
                <div className="relative flex items-baseline justify-between border-t border-border pt-4 font-mono text-xs">
                  ...
                  <span style={{ color: s.tone }} className="text-2xl font-display">{s.lines.toLocaleString()}</span>
                </div>
              </motion.div>
            </TiltCard>
          ))}
        ...
        <div className="grid md:grid-cols-[1fr_1.4fr] gap-0 border border-border bg-card/70">
          <div className="p-10 border-r border-border">
            ...
            <p className="text-bone/60 leading-relaxed mb-8">
              The generator doesn't dump a swagger viewer. It writes prose, code,
              callouts, and diagrams as a real site — searchable, indexable,
              theme-able. Or as a 300-page PDF if your CTO still mails things.
            </p>
      ...
      <section className="border-t border-border bg-card/30">
        <div className="max-w-[1400px] mx-auto px-6 py-16 grid md:grid-cols-3 gap-8">
          ...
          <div className="text-right">
            ...
            <Link to="/ai/mock-server" className="font-display text-2xl uppercase hover:text-warn transition">AI.Mock · server →</Link>
          </div>
        </div>
      </section>
    </div>
