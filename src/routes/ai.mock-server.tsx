...
export const Route = createFileRoute("/ai/mock-server")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Spin up a realistic fake of any API in one click. Faker-backed payloads, latency profiles, failure injection." },
    ],
  }),
...
const PROFILES = [
  { code: "RLZ", title: "Realistic", desc: "Faker-backed payloads: names, emails, ULIDs, ISO dates, locale-aware.", reqs: "12.4k/s", tone: "var(--acid)" },
  { code: "LAT", title: "Latency Shape", desc: "p50 / p95 / p99 distributions injected per route. Long-tail tunable.", reqs: "p95 118ms", tone: "var(--signal)" },
  { code: "FAI", title: "Failure Injection", desc: "5xx storms, partial writes, slow loris, connection resets — chaos on demand.", reqs: "0.4% / 4% / 40%", tone: "var(--warn)" },
  { code: "STA", title: "Stateful", desc: "POST /x then GET /x/{id} actually returns what you wrote. Resets on demand.", reqs: "in-memory · 50 MB", tone: "var(--acid)" },
  { code: "AUT", title: "Auth Shadow", desc: "Mimics OAuth2, bearer, API-key, mTLS — including the way real servers reject.", reqs: "401 / 403 / 419", tone: "var(--signal)" },
  { code: "WBH", title: "Webhook Replay", desc: "Fires outbound webhooks to your dev box on schedule or trigger.", reqs: "burst · drip · scheduled", tone: "var(--warn)" },
];
...
const STREAM = [
  ...
  { t: "00:00.41", line: "✦ provisioning edge worker · us-east · eu-west · ap-south", tone: "var(--signal)" },
  { t: "00:00.62", line: "✦ wiring latency profile · p95 = 118ms · jitter 22ms", tone: "var(--signal)" },
  { t: "00:00.78", line: "✦ chaos hooks armed · 0.4% 5xx · 0.1% reset", tone: "var(--warn)" },
  { t: "00:00.91", line: "✓ live @ https://m-9af3.apiguard.dev · 247 routes · stateful", tone: "var(--acid)" },
];
  ...
  return (
    <div className="relative bg-transparent text-foreground">
      ...
      <header className="fixed top-0 inset-x-0 z-50 border-b border-border/60 bg-background/70 backdrop-blur-xl">
        <div className="flex items-center justify-between px-6 py-3 font-mono text-xs tracking-widest uppercase">
          <div className="flex items-center gap-3">
            ...
            <Link to="/" className="text-bone hover:text-acid">APIGUARD/<span className="text-warn">ai.mock</span></Link>
            <span className="text-bone/40 hidden md:inline">// mock server v1.9 · edge-deployed · 47ms</span>
          </div>
          <Link to="/" className="border border-bone/30 text-bone/70 px-3 py-1 hover:border-acid hover:text-acid transition">
            ← return to OS
          </Link>
        </div>
      </header>
      ...
      <section ref={heroRef} className="relative min-h-[110vh] grid-bg overflow-hidden pt-28">
        ...
        <motion.div style={{ rotate: rotGlyph }} className="absolute right-[-180px] top-[20%] size-[520px] border border-warn/30 rounded-full pointer-events-none" />
        <motion.div style={{ rotate: rotGlyph2 }} className="absolute right-[-80px] top-[28%] size-[360px] border border-acid/20 rounded-full pointer-events-none" />

        <motion.div style={{ y: yTitle, opacity: opTitle }} className="relative px-6 max-w-[1400px] mx-auto pt-12">
          <div className="font-mono text-xs text-warn uppercase tracking-[0.3em] mb-6">[ ai.module · 03/03 · one-click mock server ]</div>
          ...
          <p className="mt-10 max-w-2xl text-bone/60 text-lg leading-relaxed">
            ...
            stable HTTPS URL — so your frontend, your tests, and your demos
          ...
          <div className="mt-12 flex flex-wrap items-center gap-4">
            ...
            <span className="font-mono text-xs text-bone/40">expected · ~1s · 247 routes · 6 regions · auto-TLS</span>
          </div>
        </motion.div>
            ...
            <div className="grid md:grid-cols-[1fr_auto_1fr] gap-0">
              <div className="p-6 font-mono text-xs space-y-3">
                ...
                <div className="flex flex-wrap gap-2">
                  {REGIONS.map((r, i) => (
                    <span key={r} className={`border px-2 py-1 cursor-pointer transition ${i < 3 ? "border-warn text-warn" : "border-bone/20 text-bone/40 hover:border-bone/60 hover:text-bone/70"}`}>
                      {r}
                    </span>
                  ))}
                </div>

                <div className="text-bone/40 pt-4">// 3. chaos · {(chaos * 100).toFixed(1)}% 5xx</div>
                <input
                  type="range"
                ...
                <div className="flex justify-between text-[10px] text-bone/40">
                  <span>calm</span><span>angry</span><span>apocalypse</span>
                </div>
              ...
              <div className="p-6 font-mono text-xs space-y-1 bg-background/40 max-h-[360px] overflow-hidden">
                ...
                <div className="text-warn">▸ awaiting deploy<span className="cursor-blink">_</span></div>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="relative px-6 max-w-[1400px] mx-auto py-32">
        <div className="flex items-end justify-between mb-12 flex-wrap gap-4">
          <div>
            <div className="font-mono text-xs text-warn uppercase tracking-[0.3em] mb-3">[ 02 · profiles ]</div>
            <h2 className="font-display text-5xl md:text-7xl uppercase tracking-tighter leading-[0.9]">six dials.<br/><span className="text-stroke">one fake stack.</span></h2>
          </div>
        ...
        <div className="grid md:grid-cols-3 gap-4">
          {PROFILES.map((p, i) => (
            <TiltCard key={p.code} className="group">
              ...
              >
                <div className="absolute top-0 right-0 font-mono text-[10rem] leading-none text-bone/[0.03] select-none">{p.code}</div>
                <div className="relative flex items-start justify-between mb-8">
                  <span className="font-mono text-xs uppercase tracking-widest" style={{ color: p.tone }}>[ {p.code} ]</span>
                  <span className="size-2 rounded-full animate-pulse" style={{ background: p.tone }} />
                </div>
                <h3 className="relative font-display text-3xl uppercase tracking-tight mb-3">{p.title}</h3>
                ...
                <div className="relative flex items-baseline justify-between border-t border-border pt-4 font-mono text-xs">
                  ...
                  <span style={{ color: p.tone }} className="font-display text-lg">{p.reqs}</span>
                </div>
              </motion.div>
            </TiltCard>
          ))}
      ...
      <section className="relative px-6 max-w-[1400px] mx-auto pb-32">
        <div className="font-mono text-xs text-warn uppercase tracking-[0.3em] mb-3">[ 03 · live.routes ]</div>
        <h2 className="font-display text-5xl md:text-6xl uppercase tracking-tighter leading-[0.9] mb-10">247 routes. all answering.</h2>
        ...
        <div className="border border-border bg-card/50">
          ...
          {ROUTES.map((r, i) => (
            ...
            >
              <div className="p-3" style={{ color: r.tone }}>{r.m}</div>
      ...
      <section className="relative px-6 max-w-[1400px] mx-auto pb-32">
        <div className="grid md:grid-cols-[1fr_1.4fr] gap-0 border border-border bg-card/70">
          <div className="p-10 border-r border-border">
            <div className="font-mono text-xs text-warn uppercase tracking-[0.3em] mb-4">[ 04 · hit it ]</div>
            <h3 className="font-display text-4xl md:text-5xl uppercase tracking-tighter leading-[0.9] mb-6">curl it now.<br/>it answers.</h3>
          ...
          <div className="bg-background/60 font-mono text-xs overflow-hidden">
            <div className="flex border-b border-border">
              {["request.sh", "response.json", "headers.txt"].map((f, i) => (
                <div key={f} className={`px-4 py-3 border-r border-border ${i === 0 ? "bg-card text-warn" : "text-bone/40"}`}>{f}</div>
              ))}
            </div>
      ...
      <section className="border-t border-border bg-card/30">
        <div className="max-w-[1400px] mx-auto px-6 py-16 grid md:grid-cols-3 gap-8">
          <div>
            ...
            <Link to="/ai/test-synth" className="font-display text-2xl uppercase hover:text-signal transition">← AI.Tests · synthesizer</Link>
          </div>
          <div className="text-center">
            <div className="font-mono text-xs text-bone/40 uppercase tracking-widest mb-2">return</div>
            <Link to="/" className="font-display text-2xl uppercase hover:text-acid transition">// apiguard.os</Link>
          </div>
          <div className="text-right">
            <div className="font-mono text-xs text-bone/40 uppercase tracking-widest mb-2">also</div>
            <Link to="/ai/docs-gen" className="font-display text-2xl uppercase hover:text-acid transition">AI.Docs · generator →</Link>
          </div>
        </div>
      </section>
    </div>
